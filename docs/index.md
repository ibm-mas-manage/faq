...
|CR<br>(Custom Resource)|A resource implemented through the Kubernetes CustomResourceDefinition <br>API. A resource is an endpoint in the Kubernetes API that stores a collection <br>of API objects of a certain kind; for example, the built-in pods resource <br>contains a collection of Pod objects. A custom resource is distinct from<br> the built-in Kubernetes resources, such as the pod and service resources.<br> Every CR is part of an API group.|Manage CRs: <br>  * ManageApp <br>  * ManageWorkspace <br>  * ManageBuild <br>  * ManageDeployment <br>  * ManageAppStatus<br> * ManageServerBundles <br> <br>  * BuildDataInterpreter (for ACM)|
